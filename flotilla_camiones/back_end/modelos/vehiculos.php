<?php
include('crud.php');
class Vehiculos implements CRUD{
    public $id;
    public $capacidad;
    public $modelo;
    public $matricula;
...
try{
 $sql = "INSERT INTO vehiculos (capacidad, modelo, matricula, color, poliza,estatus,foto) 
...
$this->con=null;
    ...
    public function actualizar(){
        try{
        $sql= "UPDATE vehiculos SET capacidad=:capacidad, modelo=:modelo, matricula=:matricula, poliza=:poliza, estatus=:estatus, costo_mensual=:costo_mensual, foto=:foto 
        WHERE id=:id";
    $stmt = $this->con->prepare($sql);
       $stmt->bindParam(':capacidad', $this->capacidad);
       $stmt->bindParam(':modelo', $this->modelo);
